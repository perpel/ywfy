...
TANGER_OCX_OBJ.OpenFromURL ("<?=\Yii::$app->request->getHostInfo()?>/uploads/report/test.doc");
...
<script language="JScript" for="TANGER_OCX" event="OnCustomMenuCmd(menuIndex,menuCaption,menuID)">
    ...
    if( menuID == 1 ){
        var a = TANGER_OCX_OBJ.SaveToURL ("./index.php?r=input/report/add&uid=<?=$uid?>&type=<?=$type?>&name=<?=$name?>", "DocContent", "", "test.doc",0);
        alert(a);
    }
</script>
<form>
