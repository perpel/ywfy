...
<input type="hidden" disabled id="action" value="bust">
<?= $form->field($model, 'EntrustCycle', ['template'=>'{input}{error}'])->hiddenInput(["data-entrust-cycle"=>"entrust-cycle", "readonly"=>"readonly"]) ?>
...
<tr>
  ...
  <tr>
   ...
   <tr>
    ...
    <td><?= $form->field($model, 'ChoiceWay', ['template'=>'{input}{error}'])->dropDownList(Common::selectedMode()) ?></td>
    </tr>
  ...
  <tr>
    <td><?= $form->field($model, 'LiquidationDate', ['template'=>'{label}'])?></td>
    <td colspan="3"><?= $form->field($model, 'LiquidationDate', ['template'=>'{input}{error}'])->textInput(["class"=>"Wdate", "onfocus"=>"WdatePicker()"])?></td>
